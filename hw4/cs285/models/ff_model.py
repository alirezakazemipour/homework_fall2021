...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = obs_unnormalized + normalize(delta_pred_normalized, delta_mean, delta_std)  # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        statistics = {k: ptu.from_numpy(v) for k, v in data_statistics.items()}
        prediction, _ = self(obs, acs,
                             *statistics.values()
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        statistics = {k: ptu.from_numpy(v) for k, v in data_statistics.items()}
        _, pred = self.forward(observations, actions, *statistics.values())
