...
from hw5.cs285.critics.dqn_critic import DQNCritic
from hw5.cs285.critics.cql_critic import CQLCritic
...
from hw5.cs285.policies.argmax_policy import ArgMaxPolicy
from hw5.cs285.infrastructure.dqn_utils import MemoryOptimizedReplayBuffer
from hw5.cs285.exploration.rnd_model import RNDModel
from .dqn_agent import DQNAgent
from hw5.cs285.policies.MLP_policy import MLPPolicyAWAC
import numpy as np
import torch
...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        qa_value = critic.qa_values(ptu.to_numpy(obs))
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        if self.agent_params['discrete']:
            ...
            for _ in range(n_actions):
                raise NotImplementedError(f"Continuous action space is not supported by AWAC in this code")
        v_pi = (torch.Tensor(vals).t().cuda() * dist.probs).sum(-1)

        # TODO Calculate Q-Values
        q_vals = self.get_qvals(self.exploitation_critic, ob_no, ptu.to_numpy(ac_na).astype(int))
        # TODO Calculate the Advantage        
        return q_vals - ptu.to_numpy(v_pi)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            self.awac_actor.update(ob_no, ac_na, adv_n)
